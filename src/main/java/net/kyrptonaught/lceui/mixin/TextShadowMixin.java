...
public class TextShadowMixin {
    ...
    private float modifyX(float original, @Local(name = "n") float n, @Local(name = "style") Style style) {
        return LCEUIMod.getConfig().closerTextShadows ? original - (((style.getColor() == TextColor.fromFormatting(Formatting.YELLOW) ? 1.0f : 2.0f) * n)/3.0f) : original;
    }
    ...
    private float modifyY(float original, @Local(name = "n") float n) {
        return LCEUIMod.getConfig().closerTextShadows ? original - ((2.0f * n)/3.0f) : original;
    }
    ...
    private float modifyRed(float original) {
        return LCEUIMod.getConfig().closerTextShadows && this.shadow ? 0.0f : original;
    }
